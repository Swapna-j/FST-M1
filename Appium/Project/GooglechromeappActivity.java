...
public class GooglechromeappActivity {
	...
	public void googleChromeActivity() throws InterruptedException {
		...
		List<MobileElement> clearElem=driver.findElementsByXPath("//android.view.View[@resource-id=\"tasksCard\"]/android.view.View/android.widget.TextView");
		for(int i=0;i<clearElem.size();i++) {
			if(clearElem.get(i).getText().trim().equalsIgnoreCase("Clear List")) {
				clearElem.get(i).click();
			}
		}
		try {
			MobileElement element2=driver.findElementByXPath("//android.view.View[@resource-id=\"tasksList\"]");
	...
	public MobileElement scrollUntilElementFound() {
		...
		while(!flag) {
			...
			try {
				List<MobileElement> elemList=driver.findElementsByXPath("//android.widget.TextView");
				for(int i=0;i<elemList.size();i++) {
					if(elemList.get(i).getText().equalsIgnoreCase("To-Do List")) {
						System.out.println("Element found Successfully");
						elemt= elemList.get(i);
						flag=true;
						break;
					}
				}
